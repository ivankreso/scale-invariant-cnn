...
--                   {240,112}}
local img_sizes = {{1504,672}, {688,304}, {240,112}}
...
local root_dir = '/home/kivan/datasets/Cityscapes/torch/'
...
local function PrepareDataset(name, max_in_file, has_gt)
  ...
  local labels = {}
  local filenames = {}
  local img_list = torch.load(root_dir .. name .. '_img_list.t7')
  ...
  for i = 1, idx_shuffle:size(1) do
    ...
    --local scale_routing = PrecomputeScaleRoutingBaseline(depth_img, baseline, img_list[idx][1], nil)
    local scale_routing = PrecomputeScaleRoutingBaseline(depth_img, baseline, img_list[idx][1], debug_dir)
