...
local img_sizes = ComputePyramidResolutions(1248, 384, 1.3, 8)
local net_out_sizes = {}
...
local target_width = img_sizes[1][1] / target_scale
local target_height = img_sizes[1][2] / target_scale
...
local in_dir = "/mnt/ikreso/datasets/KITTI/semantic_segmentation/"
local out_dir = in_dir .. '/torch/pyramid_scale_' .. net_s_factor .. '/'
...
local function PrecomputeScaleRouting(depth_img, filename)
...
local function PrepareDataset(img_dir, out_dir, name, label_colors)
  ...
  --local imglist = torch.load(img_dir .. "/img_list.t7")
  ...
  --for i = 1, 4 do
    ...
    table.insert(labels, {label_img:view(first_height, first_width), class_weights})
    local scale_routing = PrecomputeScaleRouting(depth_img, imglist[i])
    ...
    for j = 1, #img_sizes do
      ...
      --print(imglist[i])
      table.insert(pyr_data, norm_img:view(num_channels, height, width))
      --table.insert(labels, label_pyramid[j]:view(height, width))
    end
    table.insert(data, {pyr_data, scale_routing})
