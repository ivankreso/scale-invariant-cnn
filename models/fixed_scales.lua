...
--param_data_dir = '/mnt/ikreso/datasets/Cityscapes/1952x864/torch_pyramid/'
--param_data_dir = '/mnt/ikreso/datasets/Cityscapes/pyramid/1504x672_8s_1scale/'
...
param_data_dir = '/home/kivan/datasets/Cityscapes/torch/pyramid/1504x672_baseline/'
...
--local relu1_1 = cudnn.ReLU(true)(AddBatchNormalization(conv1_sz, conv1_1))
local relu1_1 = cudnn.ReLU(true)(conv1_1)
--local conv1_2 = cudnn.SpatialConvolution(conv1_sz, conv1_sz, k, k, s, s, p, p)(relu1_1)
local conv1_2 = Convolve(conv1_sz, conv1_sz, k, s, p, relu1_1)
--local relu1_2 = cudnn.ReLU(true)(AddBatchNormalization(conv1_sz, conv1_2))
local relu1_2 = cudnn.ReLU(true)(conv1_2)
local pool1 = cudnn.SpatialMaxPooling(2, 2)(relu1_2)

--local conv2_1 = cudnn.SpatialConvolution(conv1_sz, conv2_sz, k, k, s, s, p, p)(pool1)
local conv2_1 = Convolve(conv1_sz, conv2_sz, k, s, p, pool1)
--local relu2_1 = cudnn.ReLU(true)(AddBatchNormalization(conv2_sz, conv2_1))
local relu2_1 = cudnn.ReLU(true)(conv2_1)
--local conv2_2 = cudnn.SpatialConvolution(conv2_sz, conv2_sz, k, k, s, s, p, p)(relu2_1)
local conv2_2 = Convolve(conv2_sz, conv2_sz, k, s, p, relu2_1)
--local relu2_2 = cudnn.ReLU(true)(AddBatchNormalization(conv2_sz, conv2_2))
...
--local pool4 = cudnn.SpatialMaxPooling(pool_k, pool_k, pool_s, pool_s, pool_p, pool_p)(relu4_3)
...
--local relu5_1 = cudnn.ReLU(true)(conv5_1)
...
--local relu5_2 = cudnn.ReLU(true)(conv5_2)
...
--local pool5 = nn.JoinTable(2)({upsample5, relu4_3})
--local concat_size = 3*(conv5_sz+conv4_sz)
...
--local pool5 = cudnn.SpatialAveragePooling(pool_k, pool_k, pool_s, pool_s, pool_p, pool_p)(relu5_3)
--local pool5 = relu5_3

--local upsample = nn.SpatialReSamplingEx{owidth=width, oheight=height,
--			                yDim=3, xDim=4, mode='simple'}(pool5)

--local net = nn.gModule({conv1_1}, {pool3, pool4, pool5})
...
--local fc6_conv = cudnn.SpatialConvolution(3*conv5_sz, fc_sz, 1, 1)(mux)
--local fc6_conv = cudnn.SpatialConvolution(2*conv5_sz, fc_sz, 1, 1)(mux)
--local fc6_conv = cudnn.SpatialConvolution(2*(conv5_sz+conv3_sz), fc_sz, 1, 1)(mux)
--local fc6_conv = cudnn.SpatialConvolution(3*(conv5_sz+conv3_sz), fc_sz, 1, 1)(mux)
local fc6_conv = Convolve(concat_size, fc_sz, k2, s, p2, mux)
--local fc6_conv = Convolve(concat_size, fc_sz, k, s, p, mux)
...
--local fc7_conv = Convolve(fc_sz, fc_sz, k, s, p, fc6)
...
--local score = Convolve(fc_sz, num_output, k, s, p, fc7)
--local score = cudnn.SpatialConvolution(fc_sz, num_output, 1, 1)(fc6)
...
--local final_net = nn.gModule(nets, {final_score})
...
--return shared_net:cuda(), loss:cuda(), train_container, validation_container
