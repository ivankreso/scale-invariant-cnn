if isdefined_data_container_multifile_threaded then
  return
end
isdefined_data_container_multifile_threaded = true
...
local DataContainerMultiFileThreaded = torch.class('DataContainerMultiFileThreaded')

function DataContainerMultiFileThreaded:__init(...)
  local args = dok.unpack(
  {...},
  'InitializeData',
  'Initializes a DataContainerMultiFileThreaded',
  ...
  {arg='iters_between_val', type='number', help='Size of epoch', default = 1}
  )
  ...
  self.iters_between_val = args.iters_between_val
...
function DataContainerMultiFileThreaded:ReadNextFile()
  --pool:synchronize()
  --print('press')
  --io.read()
  pool:terminate()
...
function DataContainerMultiFileThreaded:GetNextBatch()
  if self.img_cnt == self.num_imgs then
  --if self.img_cnt == 30 then
    self:ReadNextFile()
  ...
  if prefix == 'train' then
    if self.epoch_iter % self.iters_between_val == 0 then
...
function DataContainerMultiFileThreaded:size()
  if prefix == 'train' then
    return 2975
  else
    return 500
...
function DataContainerMultiFileThreaded:PrintProgress()
  xlua.progress(self.epoch_iter, self:size())
end


function DataContainerMultiFileThreaded:FreeMemory()
  self.data = nil
  self.labels = nil
  self.filenames = nil
  collectgarbage()
